...
replay_faster_bp = Blueprint("replay_faster", __name__, url_prefix="/api")
...
@replay_faster_bp.route("/replay_faster/", methods=["POST"])
@replay_faster_bp.response(200, ReplayResponseSchema)
@replay_faster_bp.response(400, ErrorSchema)
@replay_faster_bp.response(500, ErrorSchema)
def replay_faster():
    """
    Start a faster replay without respecting timestamps.
    ...
    """
    current_app.logger.info("Faster replay request received")
    
    try:
        packets, iface, sid = setup_request(request)
        socketio.start_background_task(replay_loop, packets, iface, sid)
        current_app.logger.info("Faster replay started: %d packets on %s", len(packets), iface)
        return jsonify({
